...
class queue1{
    public:
        char fname[50];           //To store First name
        char lname[50];           //To store Last name
        int age,d,m,y;            //To store age,date,month,year
        queue1* next;
        queue1 *front = NULL;      //initializing front and rear equal to null
        queue1 *rear = NULL;

        //default constructor to initialize the class members of class queue1
        queue1(){
            strcpy(fname ,"xxx");
            strcpy(lname ,"xxx");
            age = 0;
...
void enqueue(char ufname[],char ulname[], int uage, int id, int um, int iy) {
            queue1* newNode;
...
class stack1
 {
    public:
        ...
        //constructor to initialize data members of class stack1
        stack1() {
            strcpy(fname, "xxx");
            strcpy(lname, "xxx");
            age =0;
...
void push(char ufname[],char ulname[], int uage, int id, int um, int iy) {
...
};     //class stack1 ends here

class list1*head=NULL;      //global declarartion of head
...
public:
    ...
    //constructor to initialize data members of class list1
list1(){
        strcpy(fname,"xxx");
        strcpy(lname,"xxx");
        age =0;
...
void insert1(list1** head_ref,char ufname[],char ulname[], int uage, int id, int um, int iy) {
...
list1* divide(list1* head,list1* end1,list1** newHead,list1** newEnd)
    {
        list1* pivot = end1;
...
list1* quickSortRecur(list1* head, list1* end1)
    {
        ...
        list1* newHead = NULL, * newEnd = NULL;
         list1* pivot = divide(head, end1, &newHead, &newEnd);
...
tree* CreateNode(char ufname[],char ulname[], int uage, int id, int um, int iy){
...
tree* insert1(tree* root, char ufname[],char ulname[], int uage, int id, int um, int iy){
            tree* newnode = CreateNode(ufname,ulname,uage,id,um,iy);
            tree* x = root;
            tree* y = NULL;
            while (x != NULL) {
                y = x;
...
void gr(){                 //function to implement graphics
...
setcolor(13);
outtextxy(250,250,"ARAMREDDY HARICHANDANA - 009");
setcolor(11);
outtextxy(250,300,"AKASH KOOTTUNGAL - 010");
setcolor(14);
outtextxy(250,350,"THEJASWIN S - 029");
setcolor(12);
outtextxy(250,400,"SAI NIHARIKA - 040");
getch();
}
//main function
int main()
...
stringstream ss;
string fname,lname;
string strage,strd,strm;
string stry,filepath;
int age,d,m,y,coun=0;
char n1[50],n2[50];
ifstream file;
queue1 q,objq;                    //creating objects
     stack1 s, objs;
     tree t, objt;
list1 l, objl;

cout<<"\n\n\n\n\n\n\n\n\n\n\t\t\t\t\t";
cout<<"DSA ASSIGNMENT-3";
for(int i=0;i<7;i++)
    cout<<endl;
cout<<"\t\t\t\tPress any key to continue..."<<endl;
    _getch();
    clrscr();
    filepath="input.txt";     //setting file path

    file.open(filepath);            //opening file path
    if (file.is_open()) {
...
while (true) {
        ...
        cout << "\nPress any key to transfer the elements from a queue into a tree..."<<endl;
        _getch();
       // clrscr();
...
while (i--) {
    ...
    if(i==coun-1)
        ...
        cout << "\nPress any key to transfer all the elements from tree to a linked list..."<<endl;
        _getch();
        //clrscr();
        //transfer the elements from the tree to a linked list
        t.remove1(root, l);
        cout << "\nsuccessfully transfered"<<endl;
...
cout<<"\t\tadd one more element\n";
    ...
    cin>> y;
        ...
        cout<<"\n ERROR9102940 cant access the file ";
    return 0;
}
